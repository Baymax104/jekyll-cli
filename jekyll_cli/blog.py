...
class __Blog:
    ...
    def __initialize_items(self, type_: BlogType) -> Dict[str, Item]:
        ...
        items = {}
        for item_path in [f for f in parent_dir.iterdir() if path_filter(self.__mode, f)]:
            name = split_filename(item_path.stem)[1] if self.__mode == 'single' else item_path.stem
