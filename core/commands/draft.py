...
class DraftCommand(Command):
    ...
    def item_action(self, args):
        ...
        item_dir = os.path.join(self.draft_dir, filename)
        ...
        with open(os.path.join(item_dir, filename), 'w', encoding='utf-8') as f:
            f.write('---\n')
            yaml.dump(self.draft_formatter, f)
            f.write('---\n')

        print(f'{os.path.join(item_dir, filename)} created as draft successfully.')

        if args.open:
            print('Opening draft...')
            os.system(f'start {os.path.join(item_dir, filename)}')
